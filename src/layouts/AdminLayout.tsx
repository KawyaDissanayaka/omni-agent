...
export default function AdminLayout() {
  ...
  return (
    <div className="flex h-screen bg-slate-100 text-gray-800 font-sans">
      {/* === SIDEBAR === */}
      <aside className="w-[280px] bg-[#F5F8FF] text-slate-800 flex flex-col fixed h-full top-0 border-r border-slate-200 z-20">
        
        {/* Logo Header */}
        <div className="pt-6 pb-4 px-6 border-b border-slate-200/80">
        ...
        <div className="px-4 pt-4">
          <button
            onClick={() => navigate(isAdminMode ? "/dashboard" : "/admin/dashboard")}
      ...
      <div className="flex-1 ml-[280px] flex flex-col min-h-screen">
        {/* Top Navbar Header */}
        <header
          className="h-14 flex items-center justify-between px-8 z-10 sticky top-0 border-b border-indigo-100"
          style={{
        ...
        >
          <div className="flex items-center gap-4">
            <div className="relative w-80">
              ...
              <input
                type="text"
                placeholder="Search agents, channels, settings..."
                className="w-full h-8 pl-9 pr-4 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500/20"
              />
            </div>
          </div>
